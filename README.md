...
Cloud Run 1サービスに「フロントエンド配信・API・非同期ワーカー」をすべて集約し、ラウンドごとのエージェント処理は Pub/Sub のイベント駆動で連鎖させています。

```mermaid
flowchart TD
    subgraph Browser["ブラウザ"]
        UI["React SPA (Dashboard.jsx)<br/>Vite でビルドした静的ファイル"]
    end

    subgraph Run["Cloud Run: social-risk-simulator (単一コンテナ)"]
        API["FastAPI (main.py)<br/>/simulate /agents /trends /simulation/*"]
        Push["POST /pubsub/push<br/>Pub/Sub Push 受信エンドポイント"]
        Logic["agent_worker.py<br/>process_agent_message()"]
    end

    Topic[("Cloud Pub/Sub<br/>topic: social-chaos-monkey-simulation")]
    DB[("Firestore<br/>agents / simulations / sessions / simulation_progress")]
    Gemini["Vertex AI<br/>Gemini 2.5 Flash"]

    UI -- "① POST /simulate<br/>(シナリオ・ラウンド数・介入設定)" --> API
    API -- "② Round1メッセージを<br/>エージェント数ぶんpublish" --> Topic
    Topic -- "③ Push配信" --> Push
    Push --> Logic
    Logic -- "④ ペルソナ・文脈を渡して行動決定" --> Gemini
    Gemini -- "RETWEET / REPLY / IGNORE + 内心のLogic" --> Logic
    Logic -- "⑤ 結果を保存" --> DB
    Logic -- "⑥ Round内全員完了なら<br/>次Roundをpublish（連鎖）" --> Topic
    UI -- "⑦ 3秒間隔でpolling<br/>GET /simulation/:id/summary" --> API
    API -- "読み書き" --> DB
    API -- "完了後: GET /simulation/:id/analysis" --> Gemini
```

### 処理の流れ
1. ブラウザから `POST /simulate` を送ると、FastAPI が Firestore から全エージェントを取得し、**Round 1 用のメッセージをエージェントの人数ぶん** Pub/Sub にpublishします（各エージェント1メッセージ＝並列処理の単位）。
2. Pub/Sub の **Push Subscription** が、同じCloud Runサービスの `POST /pubsub/push` にメッセージを配信します。
3. 受け取ったメッセージは `agent_worker.py` の `process_agent_message()` に渡り、そのエージェントのペルソナ・過去の発言・フォロー先の反応・（設定されていれば）介入用の公式声明を文脈として **Vertex AI Gemini 2.5 Flash** に渡し、`RETWEET / REPLY / IGNORE` とその内心の理由（Logic）をJSONで受け取ります。
4. 結果は Firestore の `simulations` コレクションに保存されます。同時に `simulation_progress` ドキュメントを `Firestore Increment` でカウントアップし、**そのRoundの全エージェントの処理が完了したタイミングで次Round分のメッセージを自動publish**することで、ラウンドを連鎖させます（サーバーレスなので待機用の常駐プロセスは不要）。
5. フロントエンドはWebSocketを使わず、`GET /simulation/{id}/summary` を3秒間隔でポーリングして進捗を反映します（ヘッダーの **LIVE TRACKING** 表示）。
6. 全Round完了後、`GET /simulation/{id}/analysis` でGeminiに再度シミュレーション全体のログを渡し、状況要約と推奨対応策を生成します。

### なぜこの構成か
- **単一Cloud Runサービスに集約**: フロントエンド（`npm run build` の成果物）をDockerのマルチステージビルドで同じコンテナに焼き込み、FastAPIが `dist/` を静的配信することで、フロントエンド用の別ホスティングが不要になり公開URLが1つで済みます。
- **Pub/Sub PushでワーカーもCloud Run化**: 当初はプル型の常駐ワーカー（`pubsub_worker.py`）をワーカー専用プロセスとして走らせていましたが、Cloud Run上では「常駐＝インスタンスを常時起動＝課金が発生し続ける」ため、Push Subscriptionで受動的に起動する方式に変更。アクセスが無い時間はゼロスケールし、費用がほぼかかりません。
- **ローカル開発は今もプル型が可能**: `pubsub_worker.py` は `agent_worker.py` の共通ロジックを呼び出す薄いラッパーとして残してあり、ローカルではPushエンドポイントを公開せずに `python pubsub_worker.py` を起動するだけで同じ処理を動かせます。
- **完了判定にFirestoreのカウンタ**: Pub/Subはメッセージの到着順序を保証しないため、「Round内の全エージェントが処理を終えたか」を `Increment` による原子的カウントアップで検知し、次Roundの発火をトリガーしています。
